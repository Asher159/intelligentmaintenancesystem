...
//    sparkConf.set("spark.streaming.kafka.maxRatePerPartition", "20000") //设定对目标topic每个partition每秒钟拉取的数据条数,我的topic IMS有12个分区
...
//    val ssc = new StreamingContext(sc, Seconds(5)) //这里可按频率数设置
//
//    val topic = "IMS"
...
//    val valueDStream = kafkaDStream.repartition(4).transform(rdd => rdd.sortBy(record => record.key() + record.value())).map(record => {
//      val key = record.key()
...
//      println(rdd.take(1).foreach(record => println(record._2)))
//      //       删除在hdfs上保存的hfile源文件
//      //      configUtil.deletHdfsPath(sparkSession, "hdfs://192.168.1.211:8020/tmp/hbase/", true)
//      //       保存Hfile to HDFS
//      rdd.saveAsNewAPIHadoopFile(pathString, classOf[ImmutableBytesWritable], classOf[KeyValue], classOf[HFileOutputFormat2], configuration)
//      // Bulk写Hfile to HBase
